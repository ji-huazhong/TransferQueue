...
_padding_size_key = 0 # "_padding_size_key_x123d"
...
def init_predefined_dispatch_mode():
    ...
    Dispatch.register("FAKER_MEGATRON_COMPUTE_PROTO")
...
def split_args(chunk_size, *args, **kwargs):
    splitted_args = [arg.chunk(chunk_size) for arg in args]
    return splitted_args, kwargs


def dispatch_megatron_compute_proto(worker_group, *args, **kwargs):
    dp_world_size = worker_group.world_size
    new_args, new_kwargs = split_args(dp_world_size, *args)
    return new_args, new_kwargs


def collect_megatron_compute_proto(worker_group, output):
    return output
...
DISPATCH_MODE_FN_REGISTRY = {
    ...
    Dispatch.FAKER_MEGATRON_COMPUTE_PROTO:{
        "dispatch_fn": dispatch_megatron_compute_proto,
        "collect_fn": collect_megatron_compute_proto}
}
...
def register(dispatch_mode=Dispatch.ALL_TO_ALL, execute_mode=Execute.ALL, blocking=True, materialize_futures=False):
